...
def get_local_weights(residual, ksize, std=False):
    ...
    """
    func = torch.std if std else torch.var
    ...
    pixel_level_weight = (
        func(unfolded_residual, dim=(-1, -2), unbiased=True, keepdim=True)
        .squeeze(-1)
        .squeeze(-1)
    )
    ...
    if std:
        pixel_level_weight = (
            pixel_level_weight - pixel_level_weight.mean()
        ) / pixel_level_weight.std()
        pixel_level_weight = (pixel_level_weight.tanh() + 1) / 2

    return pixel_level_weight


def get_refined_artifact_map(img_gt, img_output, img_ema=None, ksize=7, std=False):
    ...
    if not std:
        ...
        residual_sr = torch.abs(img_gt - img_output)

    pixel_level_weight = get_local_weights(residual_sr.clone(), ksize, std=std)
    if not std:
        patch_level_weight = func(
            residual_sr.clone(), dim=(-1, -2, -3), keepdim=True
        ) ** (1 / 5)
        overall_weight = patch_level_weight * pixel_level_weight
        overall_weight[residual_sr < residual_ema] = 0
    else:
        overall_weight = pixel_level_weight
