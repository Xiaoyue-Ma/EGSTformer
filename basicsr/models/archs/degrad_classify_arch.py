...
class PromptIR_DC(nn.Module):
    ...
    def forward(self, lq, features):
        ...
        mixing_weights = torch.nn.functional.softmax(self.mixing_weights, dim=0)  # 顺便修复警告
        for i, feature in enumerate(features):
            # 对齐空间尺寸
            if feature.shape[-2:] != lq_feats.shape[-2:]:
                feature = F.interpolate(feature, size=lq_feats.shape[-2:], mode='bilinear', align_corners=False)
...
class PromptIR_NoImg_DC(nn.Module):
    ...
    def forward(self, lq, features):
        ...
        lq_feats = 0
        mixing_weights = torch.nn.functional.softmax(self.mixing_weights, dim=0)  # 修复警告
        for i, feature in enumerate(features):
            if i > 0 and self.downsample:
                feature = F.interpolate(feature, scale_factor=1 / (2**i))
            # 第一次 lq_feats 是0，直接用feature尺寸；后续对齐到lq_feats尺寸
            if isinstance(lq_feats, torch.Tensor) and feature.shape[-2:] != lq_feats.shape[-2:]:
                feature = F.interpolate(feature, size=lq_feats.shape[-2:], 
