...
#########################################################################
# ========== NEW: Sparse Prompt Module (SPM) Components ========== #
#########################################################################

class HOGGuidedPromptGenBlock(nn.Module):
    """
    HOG-Guided Sparse Prompt Generation Block
    Integrates HOG features for degradation-aware expert selection
    """
    def __init__(self, prompt_dim=128, prompt_len=5, prompt_size=96, 
        ...
        # Gating network enhanced with HOG
        # Input: spatial features + HOG histogram features
    ...
    def extract_hog_features(self, x):
        """Extract HOG histogram features for gating"""
        B, C, H, W = x.shape
    ...
    def forward(self, x):
        ...
        spatial_emb = x.mean(dim=(-2, -1))  # [B, C]
        
        # HOG features for degradation-aware gating
        hog_emb = self.extract_hog_features(x)  # [B, n_bins]
        
        # Concatenate spatial and HOG features
        combined_emb = torch.cat([spatial_emb, hog_emb], dim=1)  # [B, C+n_bins]
        
        # Compute prompt weights with top-k sparsity
        ...
        # Weighted combination of expert prompts
...
#########################################################################
# ========== MODIFIED: LayerNorm with DynamicTanh Support ========== #
#########################################################################
class LayerNorm(nn.Module):
    def __init__(self, dim, LayerNorm_type="WithBias", dyt_alpha=0.5):
        """
        LayerNorm wrapper with optional DynamicTanh support.
        
        Args:
            dim: Number of features
            LayerNorm_type: Type of normalization
                - 'BiasFree': Variance-only normalization
                - 'WithBias': Standard LayerNorm
                - 'DynamicTanh': Use DyT instead (NEW!)
            dyt_alpha: Initial alpha value for DyT (only used if LayerNorm_type='DynamicTanh')
        """
        ...
        if LayerNorm_type == 'BiasFree':
            ...
            # NEW: Use DynamicTanh instead of LayerNorm
            self.body = DynamicTanh(dim, alpha_init_value=dyt_alpha)
            print(f"[HOGformer] Using DynamicTanh (α={dyt_alpha}) for dim={dim}")
        else:
...
##########################################################################
#########################################################################
# ========== MODIFIED: TransformerBlock with DyT Support ========== #
#########################################################################
class TransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type, 
                 num_experts=3, dyt_alpha=0.5):
        """
        Args:
            dyt_alpha: Alpha initialization for DyT (only used if LayerNorm_type='DynamicTanh')
        """
        super(TransformerBlock, self).__init__()
        self.attn_g_spatial = Attention_DHOGSA(dim, num_heads, bias)
        
        # Modified: Pass dyt_alpha to LayerNorm
        self.norm_g = LayerNorm(dim, LayerNorm_type, dyt_alpha=dyt_alpha)
        self.ffn = FFN_DIFF(dim, ffn_expansion_factor, bias)
        self.norm_ff1 = LayerNorm(dim, LayerNorm_type, dyt_alpha=dyt_alpha)
...
##########################################################################
#########################################################################
# ========== MODIFIED: HOGformer with DyT Support ========== #
#########################################################################
...
class HOGformer(nn.Module):
    def __init__(self, 
        ...
        bias = False,
        LayerNorm_type = 'WithBias',  # Can be 'WithBias', 'BiasFree', or 'DynamicTanh'
        dual_pixel_task = False,
        # ===== NEW: SPM parameters =====
        use_spm = True,
        prompt_len = 5,
        num_experts_per_level = [2, 3, 3, 4],  # Experts for each level
        # ===== NEW: DyT parameters =====
        dyt_alpha = 0.5  # Alpha initialization for DynamicTanh
    ):
        ...
        # Print info if using DyT
        if LayerNorm_type == 'DynamicTanh':
            print(f"\n{'='*60}")
            print(f"🚀 HOGformer with DynamicTanh (DyT)")
            print(f"   - Alpha initialization: {dyt_alpha}")
            print(f"   - Total transformer blocks: {self.num_blocks_total}")
            print(f"   - Expected DyT layers: {self.num_blocks_total * 2}")
            print(f"{'='*60}\n")

        self.patch_embed = OverlapPatchEmbed(inp_channels, dim)

        # ===== Encoder with optional SPM =====
