#degradation_classification_pretrain_model.py
...
class DCPTModel(BaseModel):
    ...
    def init_training_settings(self):
        ...
        for name, module in self.net_g.named_modules():
            if hook_names in name and name.count(".") == 0:
    ...
    def feed_train_data(self, data):
        ...
        if 'label' in data:
            self.dataset_idx = data['label'].to(self.device)
    ...
    def feed_data(self, data):
        ...
        if "dataset_idx" in data:
            ...
            self.dataset_idx = data["label"].to(self.device, non_blocking=True)
        if "gt" in data:
    ...
    def optimize_parameters(self, current_iter):
        ...
        #print(f"hook count: {len(self.hook_outputs)}")
        # for idx, h in enumerate(self.hook_outputs[::-1]):
        #     print(f"hook[{idx}] shape: {h.shape}")
    ...
    def pre_test(self):
        ...
        # pad到16的倍数，覆盖pixel_unshuffle(×2)和4级下采样的需求
        factor = 16
        if h % factor != 0:
            self.mod_pad_h = factor - h % factor
        if w % factor != 0:
            self.mod_pad_w = factor - w % factor
    ...
    def dist_validation(self, dataloader, current_iter, tb_logger, save_img=False, rgb2bgr=True, use_image=True):
        if self.opt["rank"] == 0:
